...
async function testChanges() {
  try {
    ...
    console.log('Test 1: Adding new asset (TEST-ASSET-NEW)...');
    const response1 = await makeRequest('POST', '/api/assets', {
      fields: {
        'Asset': 'TEST-ASSET-NEW',
        'Asset Description': 'Test Asset for New Changes',
        'Serial number': 'SN-TEST-NEW',
        'Cost Center': '12345',
    ...
    if (response1.status === 201) {
      ...
      console.log(`   MAY STATUS: "${asset['MAY STATUS'] || ''}" (should be empty)`);
      
      if (asset.remarks === '' && (!asset['MAY STATUS'] || asset['MAY STATUS'] === '')) {
